...
export default function CartItem({ id, name, price, count, onRemove }) {
    const totalPrice = (price * count).toFixed(2);

    return (
        <div className="flex items-center justify-between gap-3">
            <img src={Product} alt={name} className="w-12 h-12 rounded-sm" />
            ...
            <button
                className="flex items-center justify-center w-6 h-6 text-dark-grayish-blue/70 cursor-pointer"
                onClick={() => onRemove(id)}>
